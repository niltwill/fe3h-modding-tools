...
def summarize_entry(entry):
    ...
    if et == 125:
        _, ce = get_param_info(et, 1, params)
        cv = format_param_value(params[0], ce, et, 1)
        return f"character={cv}"
    if et == 126:
        _, le = get_param_info(et, 1, params)
        lv = format_param_value(params[0], le, et, 1)
        return f"lord={lv}"
    if et == 127:
        _, me = get_param_info(et, 1, params)
        mv = format_param_value(params[0], me, et, 1)
        _, ce = get_param_info(et, 2, params)
        cv = format_param_value(params[1], ce, et, 1)
        return f"map_id={mv}, completion_status={cv}"
    if et == 128:
        _, fe = get_param_info(et, 1, params)
        fv = format_param_value(params[0], fe, et, 1)
        _, ce = get_param_info(et, 2, params)
        ev = format_param_value(params[1], ce, et, 1)
        return f"flag_id={fv} expected={ev}"
    if et in (36, 37, 40, 41, 42, 49, 130, 133, 138, 150):
        return ""
    if et == 16 or et == 22:
        return f"fade {params[0]} frames"
    if et == 18 or et == 105:
        _, ce = get_param_info(et, 1, params)
        return format_param_value(params[0], ce, et, 1)
    if et == 25:
        _, ce = get_param_info(et, 1, params)
        cv = format_param_value(params[0], ce, et, 1)
        return f"{cv}, context={params[2]}"
    if et == 46:
        _, fe = get_param_info(et, 2, params)
        fv = format_param_value(params[1], fe, et, 2)
        return f"flag {fv} {'set' if params[0] else 'clear'}"
    if et == 52:
        return f"layer {params[0]}"
    if et == 54:
        _, c1 = get_param_info(et, 1, params)
        v1 = format_param_value(params[0], c1, et, 1)
        _, c2 = get_param_info(et, 2, params)
        v2 = format_param_value(params[1], c2, et, 2)
        return f"{v1} ↔ {v2}, pos={params[2]}, pan={params[3]}"
    if et == 57:
        _, ce = get_param_info(et, 1, params)
        return f"stop voice: {format_param_value(params[0], ce, et, 1)}"
    if et == 66:
        _, ce = get_param_info(et, 1, params)
        return f"{format_param_value(params[0], ce, et, 1)}: table[{params[1]}]"
    if et == 67:
        _, ce = get_param_info(et, 1, params)
        return f"{format_param_value(params[0], ce, et, 1)}: angle={params[1]}, {params[2]} frames"
    if et == 80:
        _, s = get_param_info(et, 2, params)
        _, d = get_param_info(et, 3, params)
        sv = format_param_value(params[1], s, et, 2)
        dv = format_param_value(params[2], d, et, 3)
        return f"support({sv}→{dv}) {['<','<=','==','>=','>'][params[4]]} rank {params[3]}"
    if et == 82:
        return f"party count {['<','<=','==','>=','>'][params[1]]} {params[0]}, excl={params[2]}:{params[3]}:{params[4]}"
    if et == 88:
        return f"pan {params[0]}→{params[1]}, {params[2]} frames"
    if et == 90:
        return f"page {params[0]}"
    if et == 91:
        _, ce = get_param_info(et, 1, params)
        return f"look at: {format_param_value(params[0], ce, et, 1)}"
    if et == 94:
        _, s = get_param_info(et, 1, params)
        _, d = get_param_info(et, 2, params)
        return f"{format_param_value(params[0], s, et, 1)} looks at {format_param_value(params[1], d, et, 2)}, mode={params[2]}"
    if et == 95:
        _, ie = get_param_info(et, 1, params)
        iv = format_param_value(params[0], ie, et, 1)
        _, ce = get_param_info(et, 2, params)
        cv = format_param_value(params[1], ce, et, 2)
        return f"give {iv} to {cv}"
    if et == 99:
        _, re = get_param_info(et, 1, params)
        return format_param_value(params[0], re, et, 1)
    if et == 100:
        return f"pos={params[0]}, pan={params[1]}"
    if et == 109:
        return f"RGB({params[0]}, {params[1]}, {params[2]})"
    if et == 113:
        _, ce = get_param_info(et, 1, params)
        return f"{format_param_value(params[0], ce, et, 1)}: text={params[1]}, audio={params[2]}"
    if et == 114:
        return f"BGM Variant B {'enabled' if params[0] else 'disabled'}"
    if et == 124:
        return f"RNG slot {params[0]} max {params[1]}"
    if et == 129:
        return f"subtitle/portrait {'on' if params[0] else 'off'}"
    if et == 131:
        mode_str = "add" if params[0] else "remove"
        return f"{mode_str} voices: {params[1]}, {params[2]}, {params[3]}, {params[4]}"
    if et == 132:
        _, ie = get_param_info(et, 1, params)
        return f"reward: {format_param_value(params[0], ie, et, 1)}"
    if et == 134:
        return f"turn wait {params[0]} ticks"
    if et == 136:
        return f"fade {'on' if params[0] else 'off'}, {params[1]} frames"
    if et == 140:
        return f"camera pos ({params[0]}, {params[1]}, {params[2]})"
    if et in (142, 143):
        _, ae = get_param_info(et, 1, params)
        av = format_param_value(params[0], ae, et, 1)
        return f"walk-in {av} to slot {params[1]}"
    if et == 146:
        _, loc = get_param_info(et, 1, params)
        _, tod = get_param_info(et, 2, params)
        _, mon = get_param_info(et, 3, params)
        return f"{format_param_value(params[0], loc, et, 1)} {format_param_value(params[1], tod, et, 2)} {format_param_value(params[2], mon, et, 3)}"
    if et == 147:
        _, ie = get_param_info(et, 1, params)
        return f"load item: {format_param_value(params[0], ie, et, 1)}"
    if et == 149:
        return f"ambient vol {params[0]}, {params[1]} frames"
    if et == 153:
        return f"auto-advance delay {params[0]} frames"
...
class BlockItem(QGraphicsRectItem):
    ...
    def _calculate_height(self):
        ...
        if self._block_collapsed and self._collapse_summary:
            summary_text = self._collapse_summary
        else:
            summary_text = self._summary

        # Add space for summary if needed and not expanded
        if not self._expanded and summary_text:
    ...
    def paint(self, painter, option, widget=None):
        ...
        # Draw summary text (wrapped) - use the appropriate summary
        if self._block_collapsed and self._collapse_summary:
            summary_text = self._collapse_summary
            summary_color = QColor("#98C379")  # Green for collapsed
        else:
            summary_text = self._summary
            summary_color = QColor("#848B98")  # Grey for normal

        if not self._expanded and summary_text:
            painter.setFont(self._summary_font)
            painter.setPen(QPen(summary_color))
...
class BlockScene(QGraphicsScene):
    ...
    def _apply_block_collapse(self, opener_idx: int):
        ...
        # Store the original summary before overwriting with collapse summary
        block._original_summary = block._summary  # Preserve original
    ...
    def toggle_block_collapse(self, opener_idx: int):
        ...
        if opener_idx in self._collapsed_openers:
            ...
            if opener_idx < len(self._block_items):
                ...
                # Restore the original summary from the current entry data
                block._summary = summarize_entry(block.entry)
                # Recalculate height with the original summary
                new_height = block._calculate_height()
                block._height = new_height
                block._collapsed_height = new_height
                block.setRect(0, 0, block._width, new_height)
