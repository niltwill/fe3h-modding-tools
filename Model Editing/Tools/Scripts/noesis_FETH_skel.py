...
#cloth options
bComputeCloth = False		# Compute cloth data and physics' drivers + fix cloth meshes using the NUNO/NUNV sections
...
bLoadG1T = False				# Allow to choose a paired .g1t file
...
def parseG1MS(currentPosition, bs, isDefault = True):
	...
	if not hasParsedExternal:
		...
		for i in range(jointCount):
			...
			position = [bs.readFloat()*100 for j in range(3)]
			bs.read('f')
...
def LoadModel(data, mdlList):
	...
	# =================================================================
	with open(os.path.dirname(rapi.getInputName()) + os.sep + 'boneMap.bonebind',"w") as f:
		for infoID in range(len(g1m.meshInfoList)):
			f.write("sm_"+str(infoID)+"\n")
			info = g1m.meshInfoList[infoID]
			for i,bind in enumerate(g1m.boneMapList[info[2]]):
				f.write(str(i)+" "+str(bind)+"\n")
			f.write("-1 -1\n")
