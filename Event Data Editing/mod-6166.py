...
def rebuild_6166_file(json_file, output_file):
...
def main():
    if len(sys.argv) < 3:
        print("FE3H: 6166")
        print("For file: nx/event/talk_event/data/6166.bin")
        print("")
        print("Usage:")
        print("  To dump:   python mod-6166.py dump <6166.bin> [output.json]")
        print("  To build:  python mod-6166.py build <input.json> [6166.bin]")
        print("")
    ...
    if mode == "dump":
        ...
        try:
            result = parse_6166_file(input_file)
        ...
        try:
            rebuild_6166_file(input_file, output_file)
