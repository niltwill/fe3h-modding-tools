...
def rebuild_6162_file(json_file, output_file):
    ...
    with open(output_file, 'wb') as f:
        ...
        for entry in entries:
            ...
            write_u16(f, entry.get("Unk4", 0))
            write_u16(f, entry.get("Unk5", 0))
            write_u16(f, entry.get("Unk6", 0))
            write_u16(f, entry.get("Unk7", 0))
            write_u16(f, entry.get("Unk8", 0))
            write_u16(f, entry.get("Unk9", 0))
            write_u16(f, entry.get("Unk10", 0))
            write_u16(f, entry.get("Unk11", 0))
...
def main():
    if len(sys.argv) < 3:
        print("FE3H: IN_FixedCameraInfo")
        print("For file: romfs/patch4/nx/event/talk_event/data/IN_FixedCameraInfo.bin")
        print("")
        print("Usage:")
        print("  To dump:   python mod-IN_FixedCameraInfo.py dump <IN_FixedCameraInfo.bin> [output.json]")
        print("  To build:  python mod-IN_FixedCameraInfo.py build <input.json> [IN_FixedCameraInfo.bin]")
        print("")
    ...
    if mode == "dump":
        ...
        try:
            result = parse_6162_file(input_file)
        ...
        try:
            rebuild_6162_file(input_file, output_file)
