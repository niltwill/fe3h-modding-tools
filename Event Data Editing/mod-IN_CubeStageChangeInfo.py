...
def parse_6165_file(filename):
    with open(filename, 'rb') as f:
        ...
        # Read Entry Blocks (not sure if this is right)
        entries = []
        for i in range(number_of_entries):
            entry_number1 = read_u16(f)
            entry_number2 = read_u32(f)
            unk1 = read_u32(f)
            unk2 = read_u32(f)
            entries.append({
                "EntryNumber1": entry_number1,
                "EntryNumber2": entry_number2,
                "Unk1": unk1,
                "Unk2": unk2
            })
...
def rebuild_6165_file(json_file, output_file):
    ...
    with open(output_file, 'wb') as f:
        ...
        for entry in entries:
            write_u16(f, entry.get("EntryNumber1", 0))
            write_u32(f, entry.get("EntryNumber2", 0))
...
def main():
    if len(sys.argv) < 3:
        print("FE3H: IN_CubeStageChangeInfo")
        print("For file: romfs/patch4/nx/event/talk_event/data/IN_CubeStageChangeInfo.bin")
        print("")
        print("Usage:")
        print("  To dump:   python mod-IN_CubeStageChangeInfo.py dump <IN_CubeStageChangeInfo.bin> [output.json]")
        print("  To build:  python mod-IN_CubeStageChangeInfo.py build <input.json> [IN_CubeStageChangeInfo.bin]")
        print("")
    ...
    if mode == "dump":
        ...
        try:
            result = parse_6165_file(input_file)
        ...
        try:
            rebuild_6165_file(input_file, output_file)
